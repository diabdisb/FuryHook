...
namespace driver
{
	...
	enum : UINT32
	{
		...
		KERNEL_ID_GET_DIR_BASE,
		KERNEL_ID_RUNNING,
		KERNEL_ID_MOUSE_EVENT,
		KERNEL_ID_MOUSE_INNIT,
		KERNEL_GET_PROCESS_PEB,
		KERNEL_ID_HIDE_SS,
		KERNEL_ID_READVIRTUAL
	};
	...
	ULONG64 GetDirBase() {
		...
		message.id = KERNEL_ID_GET_DIR_BASE;
		message.pid = (PVOID)detail::pid;
		message.isACE = false;
		detail::send(message);

		return (ULONG64)message.out;
	}
	bool running()
	{
		_KERNEL_REQUEST message;
	...
	bool load()
	{
		*(std::uint64_t*)&detail::function = memory::get_export(xorstr_("win32u.dll"), xorstr_("NtGdiGetUFIPathname")); // NtGdiGetUFIPathname
	...
	bool read_virtual(std::uint64_t dst, void* src, ULONGLONG size)
	{
		...
		_KERNEL_REQUEST message;
		message.magic = KE_MAGIC;
		message.id = KERNEL_ID_READVIRTUAL;
		message.pid = (PVOID)detail::pid;
		message.dst = (PVOID)dst;
		message.out = src;
		message.size = size;
	...
	bool Mouse_Event(long x, long y, unsigned short flag)
	{
		...
		_KERNEL_REQUEST message;
		message.magic = KE_MAGIC;
		message.id = KERNEL_ID_MOUSE_EVENT;
		message.x = x;
		message.y = y;
		message.button_flags = flag;
		message.physhical = FALSE;
	...
	bool Mouse_innit()
	{
		...
		_KERNEL_REQUEST message;
		message.magic = KE_MAGIC;
		message.id = KERNEL_ID_MOUSE_INNIT;
		message.physhical = FALSE;
	...
	bool SetWindowAffinity(uintptr_t HWND, UINT32 value)
	{
		...
		message.magic = KE_MAGIC;
		message.id = KERNEL_ID_HIDE_SS;
		message.value = value;
		message.window_handle = HWND;
