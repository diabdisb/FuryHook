...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool label_active)
{
    ...
    float width = GetContentRegionMax().x - style.WindowPadding.x;
    ImRect rect(window->DC.CursorPos, window->DC.CursorPos + ImVec2(width, 22.f));
    ...
    auto& state = anim[id];  // initializes if not exists

    state.background = ImLerp(state.background, g.ActiveId == id ? c::keybind::background_active : c::keybind::background_inactive, g.IO.DeltaTime * 15.f);
    state.text = ImLerp(state.text, g.ActiveId == id ? c::text::text_active : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    state.slow = ImLerp(state.slow, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 15.f);

    window->DrawList->AddRectFilled(ImVec2(state.slow + rect.Min.x, clickable.Min.y), clickable.Max, GetColorU32(state.background), c::keybind::rounding);
    window->DrawList->AddRect(ImVec2(state.slow + rect.Min.x, clickable.Min.y), clickable.Max, GetColorU32(c::keybind::outline), c::keybind::rounding);

    if (label_active)
        window->DrawList->AddText(rect.Min, GetColorU32(state.text), label);

    PushClipRect(ImVec2(state.slow + rect.Min.x, clickable.Min.y), clickable.Max, true);
    ...
    if (g.ActiveId == id) {
        for (int i = 0; i < 5; i++) {
            if (io.MouseDown[i]) {
                k = i == 0 ? 0x01 : i == 1 ? 0x02 : i == 2 ? 0x04 : i == 3 ? 0x05 : 0x06;
                value_changed = true;
                ClearActiveID();
            }
        }
    ...
    if ((hovered && io.MouseClicked[1]) || (state.active && (io.MouseClicked[0] || io.MouseClicked[1]) && !state.hovered))
    ...
    if (state.alpha >= 0.01f) {
        ...
        Begin(label, NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove);
        {
            ...
            SetCursorPos(GetCursorPos() + ImVec2((86 - CalcTextSize("Hold").x) / 2, 0));
            if (Selectable("Hold", *mode == 0)) { *mode = 0; state.active = false; }

            SetCursorPos(GetCursorPos() + ImVec2((86 - CalcTextSize("Toggle").x) / 2, 0));
            if (Selectable("Toggle", *mode == 1)) { *mode = 1; state.active = false; }

            SetCursorPos(GetCursorPos() + ImVec2((86 - CalcTextSize("Always").x) / 2, 0));
            if (Selectable("Always", *mode == 2)) { *mode = 2; state.active = false; }
        }
        End();
    }
